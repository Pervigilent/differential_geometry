...
x_ = ParametricPlot3D(RESOLUTION, x, u, v, u_, v_);
y_ = ParametricPlot3D(RESOLUTION, y, u, v, u_, v_);
z_ = ParametricPlot3D(RESOLUTION, z, u, v, u_, v_);
surf(x_, y_, z_);
...
% Create 3D points for parametric plot.  
  ...
  for i = 1 : resolution_u
    for j = 1 : resolution_v
      ...
      output = [output; double(temporary)];
    end
  end
  warning('on', 'all');
end